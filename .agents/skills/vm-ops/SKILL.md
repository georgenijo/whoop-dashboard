...
- **CLI mode** (default fallback) — spawns `/usr/local/bin/Codex -p <prompt>` as a subprocess. Slow (~30-60s).

**Critical gotcha:** the Codex CLI is logged in via OAuth, but `ANTHROPIC_API_KEY` in `.env.local` overrides that and breaks the CLI with `Error: exit 1`. The route strips it before spawning:
```ts
const cleanEnv = { ...process.env };
delete cleanEnv.ANTHROPIC_API_KEY;
cleanEnv.HOME = "/home/george";
spawn("/usr/local/bin/Codex", [...], { env: cleanEnv });
```
...
If `daily_sync.py` insight step takes ~40s on a fresh VM, the `streamlit/whoop/insights.py` module is using the legacy `Codex` CLI shellout instead of the Anthropic SDK. The fix landed in commit `5096202` (April 2026) — check `head -5 streamlit/whoop/insights.py` shows `import anthropic`, not `import subprocess`. If old code is still on disk, the VM hasn't pulled main.
