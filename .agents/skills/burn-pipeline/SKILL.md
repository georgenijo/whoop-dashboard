...
- Issues need DB migration or schema change (manual deploy with snapshot — see AGENTS.md "Deploy").
...
1. Start the local server:
   ```bash
   bash ~/.Codex/skills/whoop-dev/bin/up.sh /Users/george-mac-mini/Documents/code/whoop-dashboard
   ```
...
bash ~/.Codex/skills/whoop-dev/bin/down.sh --port <port>
...
- Rollback recipe (AGENTS.md "Deploy" section: `scp` backup DB, `git checkout <prev-sha>`, rebuild, restart).
...
- Database migrations / schema changes (manual deploy with DB snapshot per AGENTS.md).
