...
with chat_tab:
    ...
    # Quick-start chips — only show when conversation is empty
    chip_prompt = None
    if not st.session_state.messages:
        st.markdown("####")
        st.markdown("**Ask me anything about your Whoop data:**")
        chips = [
        ...
        for i, chip in enumerate(chips):
            with chip_cols[i]:
                if st.button(chip, key=f"chip_{i}"):
                    ui_logger.info("CHIP_CLICK | chip=%s", chip)
                    chip_prompt = chip

    # Chat input (always pinned to bottom by Streamlit)
    user_input = st.chat_input("Ask about your Whoop data...")
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                response = send_chat_message(
                    prompt, st.session_state.messages[:-1], days
                )
