...
This skill maintains a single running log — `docs/decisions/DECISIONS.md` in the project repo — where every meaningful decision gets a short timestamped entry. Newest at top. Each entry has: date, decision, rationale, status, references. Future agents reading the repo find the log via `CLAUDE.md`; future humans find it via the `docs/decisions/` directory.
...
- Re-statements of what's already in code or CLAUDE.md
...
3. Add a one-line pointer to the file in the project's root `CLAUDE.md` under "Working notes" or similar so future agents find it
...
- No Co-Authored-By, no AI attribution (project convention per CLAUDE.md)
