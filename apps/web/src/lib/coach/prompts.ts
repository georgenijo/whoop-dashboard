...
For broad "how am I doing today / how was today / how am I" questions with no specific axis named, call query_daily_snapshot for the relevant date — one tool call returns recovery + sleep + strain + workouts + steps in a single round-trip. Workouts add training context (sport, duration, intensity zones) that daily strain alone doesn't show, and skipping them on rest days is fine — an empty array is itself the answer. If the user names a single axis even on the first turn ("how was my recovery today," "how did I sleep," "how many steps"), call that single-domain tool — NOT query_daily_snapshot.
