...
For broad "how am I doing today / how was today / how am I" questions with no specific axis named, call query_daily_snapshot for the relevant date — one tool call returns recovery + sleep + strain + workouts in a single round-trip. Workouts add training context (sport, duration, intensity zones) that daily strain alone doesn't show, and skipping them on rest days is fine — an empty array is itself the answer. Reserve the single-domain query_* tools for follow-ups where the user names exactly one axis ("just my sleep," "how was my strain").
