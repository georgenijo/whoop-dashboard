...
| Environment | macOS latest, Xcode 26.4.1 (pinned) |
...
## Project regeneration on Cloud

`Coach.xcodeproj` is gitignored — generated by xcodegen from `project.yml`. Xcode Cloud runs `ci_scripts/ci_post_clone.sh` after clone, which installs xcodegen via Homebrew and regenerates the project so the cloud build has a valid `.xcodeproj` + shared scheme.
